...
static void write_utmp()
{
    ...
    strlcpy(utent.ut_id, tty_name + 8, sizeof(utent.ut_id)+1);
    strlcpy(utent.ut_line, tty_name + 5, sizeof(utent.ut_line)+1);
    utent.ut_pid = comm_pid;
    pw = getpwuid(getuid());
    if (pw != NULL)
    {
        strlcpy(utent.ut_name, pw->pw_name, sizeof(utent.ut_name)+1);
    }
    strlcpy(utent.ut_host, XDisplayString(display), sizeof(utent.ut_host)+1);
    ...
    snprintf(utentx.ut_id, sizeof(utentx.ut_id)-1, "vt%02x", n);
    snprintf(utentx.ut_line, sizeof(utentx.ut_line)-1, "pts/%d", n);
    pw = getpwuid(getuid());
    if (pw != NULL)
    {
        strlcpy(utentx.ut_name, pw->pw_name, sizeof(utent.ut_name)+1);
    }
    strlcpy(utentx.ut_host, XDisplayString(display), sizeof(utentx.ut_host)+1);
...
#define UTMP_FILE    "/etc/utmp"
    ...
    if (tslot > 0)
    {
        ...
        if (loc_fd >= 0)
        {
            ...
            strlcpy(locent.t_name, XDisplayString(display), LN_NAMEL+1);
            locent.t_name[LN_NAMEL - 1] = '\0';
            strlcpy(locent.t_extra, "(xvt)", sizeof("(xvt)")+1);
...
int run_command(char* command, char** argv)
{
    ...
    if (fcntl(ptyfd, F_SETFL, O_NDELAY) < 0)
    {
        error("fcntl() failed\n");
        return(-1);
    }
    ...
    if (comm_pid == 0)
    {
        ...
        if (fchown(ttyfd, uid, gid) < 0)
        {
            error("failed to chown ttyfd\n");
        }
