...
void fix_environment()
{
    ...
    snprintf(buf, sizeof(buf)-1, "DISPLAY=%.400s", DisplayString(display));
    com_env[j++] = scopy(buf);
    snprintf(buf, sizeof(buf)-1, "WINDOWID=%d", (int)main_win);
    com_env[j++] = scopy(buf);
#ifdef LC_ENV
    scr_get_size(&cols, &lines);
    snprintf(buf, sizeof(buf)-1, "LINES=%d", lines);
    com_env[j++] = scopy(buf);
    snprintf(buf, sizeof(buf)-1, "COLUMNS=%d", cols);
    com_env[j++] = scopy(buf);
#endif /* LC_ENV */
    com_env[j] = NULL;
    environ = com_env;
...
void init_display(int argc, char** argv, int iargc, char** iargv, char* command)
{
     ...
     */
    snprintf(str1, sizeof(str1)-1, "%.100s.display", res_name);
    snprintf(str2, sizeof(str2)-1, "%.100s.Display", XVT_CLASS);
    if (XrmGetResource(commandlineDB, str1, str2, &str_type, &value) == True)
    {
        strlcpy(str1, value.addr, (int)value.size+1);
        display_name = str1;
    }
...
static unsigned char* get_resource(char* name, char* class)
{
    ...
    snprintf((char*)str1, sizeof(str1)-1, "%.100s.%.100s", res_name, name);
    snprintf((char*)str2, sizeof(str2)-1, "%.100s.%.100s", XVT_CLASS, class);
    if (XrmGetResource(rDB, (char*)str1, (char*)str2, &str_type, &value) == True)
    {
        ...
        strlcpy((char*)resource, value.addr, (int)value.size+1);
        return(resource);
    }

    /*  The following is added for compatibility with xterm.
     */
    snprintf((char*)str1, sizeof(str1)-1, "%.100s.vt100.%.100s", res_name, name);
    snprintf((char*)str2, sizeof(str2)-1, "%.100s.VT100.%.100s", XVT_CLASS, class);
    if (XrmGetResource(rDB, (char*)str1, (char*)str2, &str_type, &value) == True)
    {
        if (value.size > 255)
        {
